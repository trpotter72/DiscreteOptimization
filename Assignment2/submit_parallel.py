...
def compute(metadata, solver_file_override=None):
    ...
    for problem in selected_problems:
        ps = mp.Process(target=solve, args=(problem,results,solver_file_override))
        procs.append(ps)
        ps.start()
    for ps in procs:
        ps.join()
    ...
    return results._getvalue() #Used to get the underlying dict from the proxy

def solve(problem, results, solver_file_override):
